...
def calc_auprc(pred_edges, GT_edges):
    pred_edges = 1 - pred_edges[:, :, 0]

    return average_precision_score(
        GT_edges.cpu().detach().flatten(),
        pred_edges.cpu().detach().flatten(),
    )


def calc_jac(pred_edges, GT_edges):
    # pred_edges = 1 - pred_edges[:, :, 0]
    _, pred_edges = pred_edges.max(-1)
...
def create_rel_rec_send_bi(args, num_atoms):
    """Based on https://github.com/ethanfetaya/NRI (MIT License)."""
    if args.unobserved > 0 and args.model_unobserved == 1:
        num_atoms -= args.unobserved

    # Generate off-diagonal interaction graph: modified for full-graph setting
    off_diag = np.ones([num_atoms, num_atoms]) # - np.eye(num_atoms)
