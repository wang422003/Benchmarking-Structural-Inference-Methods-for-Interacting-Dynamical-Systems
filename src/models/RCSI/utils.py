...
def select_manual_4_plus_1_edges(origin_edges, fixed, manual_rest):
    """
    return the separated two sets: selected and the rest
    :param fixed:
    :param origin_edges:
    :param manual_rest:
    :return:
    """
    num_nodes = origin_edges.shape[-1]
    # all_ids = np.linspace(0, num_nodes - 1, num_nodes)
    # all_ids = all_ids.astype(int)
    # all_ids = all_ids.tolist()
    fixed.append(manual_rest)
    node_select = np.array(fixed)
    node_select = node_select.astype(int)
    node_select = node_select.tolist()
    # rest_nodes = []
    # for num in all_ids:
    #     if num in node_select:
    #         continue
    #     else:
    #         rest_nodes.append(num)
    edges_selected = origin_edges[node_select, :]
    edges_selected = edges_selected[:, node_select]

    return edges_selected


def select_partial_edges(origin_edges, start_id=1, num_partials=5, manual_4=False, manual_rest=5):

    if manual_4:
        new_edges = select_manual_4_plus_1_edges(origin_edges, [1, 2, 3, 4], manual_rest)
    else:
        assert start_id + num_partials - 1 <= origin_edges.shape[0], "Check Start_id and num_partials!!"
        new_edges = origin_edges[start_id: start_id + num_partials]
        new_edges = new_edges[:, start_id: start_id + num_partials]
    return new_edges


def manual_4_plus_1(origin_feat, fixed, manual_rest):
    """
    return the separated two sets: selected and the rest
    :param fixed:
    :param origin_feat:
    ...
    for num in all_ids:
        if num in node_select:
            ...
            rest_nodes.append(num)
    feat_selected = origin_feat[:, :, :, node_select]
    feat_rest = origin_feat[:, :, :, rest_nodes]
    return feat_rest, feat_selected


def features_split_phy2(origin_feat, start_id, num_partials, func='mean', pure_partial=False, embed_rest=False,
                        manual_4=False, manual_rest=5):

    if manual_4:
        feat_rest, feat_selected = manual_4_plus_1(origin_feat, [1, 2, 3, 4], manual_rest)
    else:
        feat_rest_1, feat_selected, feat_rest_2 = np.split(origin_feat, [start_id, start_id + num_partials], axis=-1)
        feat_rest = np.concatenate([feat_rest_1, feat_rest_2], axis=-1)
    if func == 'mean' and not embed_rest:
        feat_rest = np.mean(feat_rest, axis=-1)
        feat_rest = feat_rest[:, :, :, np.newaxis]
    if not pure_partial:
        new_feat = np.concatenate([feat_rest, feat_selected], axis=-1)
    else:
        new_feat = feat_selected
    return new_feat
...
def load_data_phy2(batch_size=1, suffix='', self_loops=True, data_portion=1.0, time_steps=49, shuffle_data=False):
...
def dis_load_data_phy2(batch_size=1, suffix='', self_loops=True, data_portion=1.0):
    loc_train = np.load('./data/physics_simulations/loc_train_' + suffix + '.npy')
    vel_train = np.load('./data/physics_simulations/vel_train_' + suffix + '.npy')
    edges_train = np.load('./data/physics_simulations/edges_train_' + suffix + '.npy')
    ...
    loc_train = portion_data(loc_train, data_portion)
    vel_train = portion_data(vel_train, data_portion)

    loc_valid = portion_data(loc_valid, data_portion)
    vel_valid = portion_data(vel_valid, data_portion)
...
def load_data_phy2_partial(batch_size=1, suffix='', self_loops=True, id_start=2, num_partial=5):
    """

    :param num_partial: The number of partial nodes to be selected
    :param id_start: The start ID of the selected nodes.
    :param batch_size:
    :param suffix:
    :param self_loops:
    :return: data_loaders for training, the data composed of averaged description of remained nodes
             and the selected nodes
    """
    loc_train = np.load('./data/physics_simulations/loc_train_' + suffix + '.npy')
    vel_train = np.load('./data/physics_simulations/vel_train_' + suffix + '.npy')
    edges_train = np.load('./data/physics_simulations/edges_train_' + suffix + '.npy')
    loc_train = features_split_phy2(
        origin_feat=loc_train,
        start_id=id_start,
        num_partials=num_partial
    )
    vel_train = features_split_phy2(
        origin_feat=vel_train,
        start_id=id_start,
        num_partials=num_partial
    )
    edges_train = select_partial_edges(
        origin_edges=edges_train,
        start_id=id_start,
        num_partials=num_partial
    )
    ...
    vel_valid = np.load('./data/physics_simulations/vel_valid_' + suffix + '.npy')
    edges_valid = np.load('./data/physics_simulations/edges_valid_' + suffix + '.npy')
    loc_valid = features_split_phy2(
        origin_feat=loc_valid,
        start_id=id_start,
        num_partials=num_partial
    )
    vel_valid = features_split_phy2(
        origin_feat=vel_valid,
        start_id=id_start,
        num_partials=num_partial
    )
    edges_valid = select_partial_edges(
        origin_edges=edges_valid,
        start_id=id_start,
        num_partials=num_partial
    )
    ...
    vel_test = np.load('./data/physics_simulations/vel_test_' + suffix + '.npy')
    edges_test = np.load('./data/physics_simulations/edges_test_' + suffix + '.npy')
    loc_test = features_split_phy2(
        origin_feat=loc_test,
        start_id=id_start,
        num_partials=num_partial
    )
    vel_test = features_split_phy2(
        origin_feat=vel_test,
        start_id=id_start,
        num_partials=num_partial
    )
    edges_test = select_partial_edges(
        origin_edges=edges_test,
        start_id=id_start,
        num_partials=num_partial
    )
    ...
    vel_train = np.transpose(vel_train, [0, 3, 1, 2])
    feat_train = np.concatenate([loc_train, vel_train], axis=3)
    edges_train = np.reshape(edges_train, [-1, (num_nodes - 1) ** 2])
    edges_train = np.array((edges_train + 1) / 2, dtype=np.int64)

    loc_valid = np.transpose(loc_valid, [0, 3, 1, 2])
    vel_valid = np.transpose(vel_valid, [0, 3, 1, 2])
    feat_valid = np.concatenate([loc_valid, vel_valid], axis=3)
    edges_valid = np.reshape(edges_valid, [-1, (num_nodes - 1) ** 2])
    edges_valid = np.array((edges_valid + 1) / 2, dtype=np.int64)

    loc_test = np.transpose(loc_test, [0, 3, 1, 2])
    vel_test = np.transpose(vel_test, [0, 3, 1, 2])
    feat_test = np.concatenate([loc_test, vel_test], axis=3)
    edges_test = np.reshape(edges_test, [-1, (num_nodes - 1) ** 2])
    edges_test = np.array((edges_test + 1) / 2, dtype=np.int64)
...
def load_data_phy2_rest_embed(batch_size=1, suffix='', self_loops=True, id_start=2, num_partial=5,
                              manual_4=False, manual_rest=5):
    """
    ...
    # [num_samples, num_timesteps, num_dims, num_nodes]
    num_nodes = num_partial  # loc_train.shape[3]
    # print("num_nodes: ")
    # print(num_nodes)
    n_train = loc_train.shape[0]
    n_test = loc_test.shape[0]
...
def load_data_phy2_pure_partial(batch_size=1, suffix='', self_loops=True, id_start=2, num_partial=5):
    """
...
def load_data_kuramoto(batch_size=1, suffix='', self_loops=True, data_portion=1.0, time_steps=49, shuffle_data=False):
...
def dis_load_data_kuramoto(batch_size=1, suffix='', self_loops=True):
    feat_train = np.load('./data/physics_simulations/feat_train_' + suffix + '.npy')
    edges_train = np.load('./data/physics_simulations/edges_train_' + suffix + '.npy')
...
def dis_load_data_netsim(batch_size=1, suffix='', self_loops=True):
    mat = scipy.io.loadmat('./data/netsims/sim' + suffix[-1] + '.mat')
...
def dis_load_data_genetic(batch_size=1, data_type='LI',
                      self_loops=False, distributed_flag=False, norm_flag=False, data_portion=1.0):
    train_traj = np.load('./data/Synthetic-H/sampled_data/' + data_type + '/train.npy')
    # shape:[num_simulations, num_genes, time_steps]
    train_traj = np.transpose(train_traj, [0, 2, 1])  # change to [num_simulations, timesteps, num_genes]
    train_traj = train_traj[..., np.newaxis]  # shape: [num_sim, timesteps, num_genes, dimension]
    train_traj = np.transpose(train_traj, [0, 1, 3, 2])  # shape: [num_sim, timesteps, dimension, num_genes]
    train_traj = portion_data(train_traj, data_portion)
    ...
    valid_traj = valid_traj[..., np.newaxis]  # shape: [num_sim, timesteps, num_genes, dimension]
    valid_traj = np.transpose(valid_traj, [0, 1, 3, 2])
    valid_traj = portion_data(valid_traj, data_portion)
...
def load_data_CRN(batch_size=1, data_type='toy5_timesteps500',
                  self_loops=False, distributed_flag=False, norm_flag=False):
    train_traj = np.load('./data/CRN/' + data_type + '/train.npy')
    # shape:[num_simulations, num_genes, time_steps]
    ...
    train_traj = train_traj[..., np.newaxis]  # shape: [num_sim, timesteps, num_genes, dimension]
    train_traj = np.transpose(train_traj, [0, 1, 3, 2])  # shape: [num_sim, timesteps, dimension, num_genes]
    edges_train = np.load('./data/CRN/' + data_type + '/edges.npy')
    edges_train = np.tile(edges_train, (n_train, 1, 1))

    valid_traj = np.load('./data/CRN/' + data_type + '/valid.npy')
    n_valid = valid_traj.shape[0]
    valid_traj = np.transpose(valid_traj, [0, 2, 1])  # change to [num_simulations, timesteps, num_genes]
    valid_traj = valid_traj[..., np.newaxis]  # shape: [num_sim, timesteps, num_genes, dimension]
    valid_traj = np.transpose(valid_traj, [0, 1, 3, 2])
    edges_valid = np.load('./data/CRN/' + data_type + '/edges.npy')
    edges_valid = np.tile(edges_valid, (n_valid, 1, 1))

    test_traj = np.load('./data/CRN/' + data_type + '/test.npy')
    n_test = test_traj.shape[0]
    test_traj = np.transpose(test_traj, [0, 2, 1])  # change to [num_simulations, timesteps, num_genes]
    test_traj = test_traj[..., np.newaxis]  # shape: [num_sim, timesteps, num_genes, dimension]
    test_traj = np.transpose(test_traj, [0, 1, 3, 2])
    edges_test = np.load('./data/CRN/' + data_type + '/edges.npy')
    edges_test = np.tile(edges_test, (n_test, 1, 1))
...
class NRMSELoss(torch.nn.Module):
    ...
    def forward(self, preds, target):
        loss = (preds - target) ** 2 + self.eps
        std = torch.std(loss)
        loss = torch.sqrt(torch.mean(loss / std ** 2))
        return loss


def total_loss(data, output, prob, args, log_prior):
    target = data[:, :, 1:, :]
    ...
    return loss, loss_nll, loss_kl, loss_de, loss_dl, loss_sl


def total_rc_loss(data, output, rc_output, prob, args, log_prior):
    target = data[:, :, 1:, :]
    ...
    loss = loss_rc + args.KL_weight * loss_kl + args.smoothness_weight * loss_de - \
           args.degree_weight * loss_dl + args.sparsity_weight * loss_sl
    return loss, loss_rc, loss_kl, loss_de, loss_dl, loss_sl


def nrmse_loss(preds, target):
    loss = (preds - target) ** 2 +  1e-6
    std = torch.std(loss)
    loss = torch.sqrt(torch.mean(loss / std ** 2))
    return loss
...
def nll_gaussian_new(preds, target, variance, add_const=False):
    # With extra values of errors of all preds
    ...
    if add_const:
        const = 0.5 * np.log(2 * np.pi * variance)
        neg_log_p += const
    return neg_log_p.sum() / (target.size(0) * target.size(1)), neg_log_p
...
def adj_thresholding(adj, threshold=0.95, keep_origin=False):
    adj = adj.squeeze()
    window = int(adj.shape[0] ** 0.25)
    adj_sort_smoothen_upper_half = (np.convolve(np.sort(adj), np.ones(window),"valid") / window)[adj.shape[0]//2:]
    threshold = adj_sort_smoothen_upper_half[np.diff(adj_sort_smoothen_upper_half).argmax()]
    # threshold = np.quantile(adj, threshold)
    if not keep_origin:
        adj[adj >= threshold] = 1
    adj[adj < threshold] = 0
    return adj
...
def edge_accuracy(preds, target, NDtest=False, topk=False, c_r=False):
    ...
    if c_r:
        ...
        return float(correct) / (target.size()[0])
    else:
        correct = preds.float().data.eq(
            target.float().data.view_as(preds)).cpu().sum()

    return np.array(float(correct) / (target.size()[0] * target.size()[1]))
...
def initialization_p(args):
    # Generate off-diagonal interaction graph
    if not args.pure_partial:
        if not args.include_self_loops:
            adj = np.ones([args.num_partial_nodes + 1, args.num_partial_nodes + 1]) - np.eye(args.num_partial_nodes + 1)

        else:
            adj = np.ones([args.num_partial_nodes + 1, args.num_partial_nodes + 1])
    else:
        if not args.include_self_loops:
            adj = np.ones([args.num_partial_nodes, args.num_partial_nodes]) - np.eye(args.num_partial_nodes)

        else:
            adj = np.ones([args.num_partial_nodes, args.num_partial_nodes])
...
def load_data(args):
    if args.suffix in ['LI', 'LL', 'CY', 'BF', 'TF', 'BF-CV']:
        train_loader, valid_loader, test_loader = load_data_genetic(
            ...
            data_portion=args.RC_portion,
            time_steps=args.RC_time_steps,
            shuffle_data=args.RC_shuffle
        )
    elif args.suffix in ['springs', 'charged']:
        train_loader, valid_loader, test_loader = load_data_phy2(
            batch_size=args.batch_size,
            suffix=args.suffix + args.suffix2,
            self_loops=args.include_self_loops,
            data_portion=args.RC_portion,
            time_steps=args.RC_time_steps,
            shuffle_data=args.RC_shuffle
    ...
    elif args.suffix in ["NetSim1", "NetSim2", "NetSim3"]:
        ...
        train_loader, valid_loader, test_loader = load_data_scRNAseq(
...
def dis_load_data(args):
    ...
    elif args.suffix in ['toy5_timesteps500',
                         'toy5_timesteps1000',
                         'toy5_timesteps500_full',
                         'toy5_timesteps1000_full']:
        train_loader, valid_loader, test_loader = load_data_CRN(
            batch_size=args.batch_size,
            data_type=args.suffix,
            self_loops=args.include_self_loops,
        )
...
def load_data_partial(args):
    if args.suffix in ['LI', 'LL', 'CY', 'BF', 'TF', 'BF-CV']:
        ...
        train_loader, valid_loader, test_loader = load_data_phy2_partial(
            batch_size=args.batch_size,
            suffix=args.suffix,
            self_loops=args.include_self_loops,
            id_start=args.start_partial_nodes,
            num_partial=args.num_partial_nodes
        )
    elif args.suffix in ['kuramoto']:
        train_loader, valid_loader, test_loader = load_data_kuramoto(
            batch_size=args.batch_size,
...
def load_data_rest_embed(args):
    if args.suffix in ['LI', 'LL', 'CY', 'BF', 'TF', 'BF-CV']:
        ...
        train_loader, valid_loader, test_loader = load_data_phy2_rest_embed(
            batch_size=args.batch_size,
            suffix=args.suffix,
            self_loops=args.include_self_loops,
            id_start=args.start_partial_nodes,
            num_partial=args.num_partial_nodes,
            manual_4=args.manual_4,
            manual_rest=args.manual_4_r
        )
    elif args.suffix in ['kuramoto']:
        train_loader, valid_loader, test_loader = load_data_kuramoto(
            batch_size=args.batch_size,
...
def load_data_pure_partial(args):
    if args.suffix in ['LI', 'LL', 'CY', 'BF', 'TF', 'BF-CV']:
        train_loader, valid_loader, test_loader = load_data_genetic(
            batch_size=args.batch_size,
            data_type=args.suffix,
        ...
        train_loader, valid_loader, test_loader = load_data_phy2_pure_partial(
            batch_size=args.batch_size,
            suffix=args.suffix,
            self_loops=args.include_self_loops,
            id_start=args.start_partial_nodes,
...
def split_rest_logits(logits, args):
    sp = logits.view(logits.size()[0], args.num_partial_nodes + 1, args.num_partial_nodes + 1, -1)
    sp = sp[:, 1:, :, :]
    sp = sp[:, :, 1:, :]
    # print(sp.size())
    sp = torch.reshape(sp, (sp.size()[0], args.num_partial_nodes ** 2, 2))
    # print(sp.size())
    return sp
...
def update_tensorboard(tf_board_writer, epoch, nll_train=None, kl_train=None,
    ...
    if prob is not None:
        ...
        # plt.matshow(prob, cmap="Blues", vmin=0, vmax=1)
        # plt.colorbar()
        # tf_board_writer.add_figure("Adjacency matrix", plt.gcf(), epoch)
        del fig, ax, im
