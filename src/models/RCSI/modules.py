...
class GRUCell(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        log.info("gate_x:")
        log.info(gate_x.size())
        gate_h = gate_h.squeeze()
        log.info("gate_h:")
        log.info(gate_h.size())
...
class MPMEncoder(nn.Module):
    ...
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True, output_tensor=False, gin=False):
        ...
        if self.factor:
            ...
            log.info("Using factor graph MLP encoder.")
        else:
            self.mlp4 = MLP(n_hid * 2, n_hid, n_hid, do_prob)
            log.info("Using MLP encoder.")
...
class GRUEncoder(nn.Module):
    ...
    def forward(self, inputs, rel_rec, rel_send):
        ...
        x_edges_1 = torch.empty(
            rel_rec.size()[1], int(x.size()[1] / rel_rec.size()[1]), x_exp.size()[2], x_exp.size()[-1]
        )
        ...
        x_concat_2 = torch.empty((
            rel_rec.size()[1], int(x.size()[1] / rel_rec.size()[1]), x_edges.size()[1], x_edges.size()[-1]))
...
class CNNEncoder(nn.Module):
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True):
        ...
        if self.factor:
            log.info("Using factor graph CNN encoder.")
        else:
            log.info("Using CNN encoder.")
...
class GINEncoderIncEmbedding(nn.Module):
    """
    Modified MLP encoder with invariance and other stuffs.
    """
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True, gin=True, func='avg', kernel_size=5):
        super(GINEncoderIncEmbedding, self).__init__()

        self.factor = factor
        self.gin = gin
        self.embed_mlp1 = MLP(n_in, n_hid, n_hid, do_prob)
        self.embed_mlp2 = MLP(n_hid, n_hid, n_in, do_prob)
        if func == 'max':
            self.embed_pool = nn.MaxPool1d(kernel_size=kernel_size)
        else:
            self.embed_pool = nn.AvgPool1d(kernel_size=kernel_size)
        self.mlp1 = MLP(n_in, n_hid, n_hid, do_prob)
        self.mlp2 = MLPEdges(n_hid * 2, n_hid * 2, n_hid, do_prob)
        self.mlp3 = MLP(n_hid, n_hid, n_hid, do_prob)
        if self.factor:
    ...
    def forward(self, inputs, rel_rec, rel_send, adj):
        # Input shape: [num_sims, num_nodes, num_timesteps, num_dims]
        num_selected = adj.shape[0] - 1
        x_raw = inputs.view(inputs.size()[0], inputs.size()[1], -1)
        # New shape: [num_sims, num_nodes, num_timesteps*num_dims]
        # print("x")
        # print(x.size())
        x = x_raw[:, -1 * num_selected:, :]
        # Embedding on the rest nodes:
        x_rest = x_raw[:, :num_selected, :]
        x_rest = self.embed_mlp1(x_rest)
        x_rest = torch.transpose(x_rest, 1, 2)
        x_rest = self.embed_pool(x_rest)
        x_rest = torch.transpose(x_rest, 1, 2)
        x_rest = self.embed_mlp2(x_rest)

        x_raw = torch.cat([x_rest, x], dim=1)
        # print(x_raw.size())
        # print(rel_rec.size())
        x = self.mlp1(x_raw)  # 2-layer ELU net per node
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
        ...
        log.info('Using learned interaction net decoder.')
    ...
    def single_step_forward(self, single_timestep_inputs, rel_rec, rel_send,
        ...
        # aug_inputs = [batches, timesteps, num_nodes, decoder-hidden + dims]
        ...
        return single_timestep_inputs + pred, aug_inputs
    ...
    def forward(self, inputs, rel_type, rel_rec, rel_send, pred_steps=1):
        ...
        last_pred, aug_inputs = self.single_step_forward(last_pred, rel_rec, rel_send,
                                             curr_rel_type)
        ...
        return pred_all.transpose(1, 2).contiguous(), aug_inputs
...
class RNNDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, n_hid,
        ...
        log.info('Using learned recurrent interaction net decoder.')
    ...
    def forward(self, data, rel_type, rel_rec, rel_send, pred_steps=1,
        ...
        for step in range(0, inputs.size(1) - 1):
            ...
            pred, hidden, aug_inputs = self.single_step_forward(ins, rel_rec, rel_send,
                                                    rel_type, hidden)
            pred_all.append(pred)

        preds = torch.stack(pred_all, dim=1)

        return preds.transpose(1, 2).contiguous(), aug_inputs
...
class GraphAttentionLayer_2(nn.Module):
    ...
    def forward(self, inputs, adj):
        ...
        log.info("h: {}".format(h.size()))
        log.info("W: {}".format(self.W.size()))
        Wh = torch.mm(h, self.W)  # h.shape: (N, in_features), Wh.shape: (N, out_features)
        log.info("Wh: {}".format(Wh.size()))
        a_input = self._prepare_attentional_mechanism_input(Wh)
        log.info("a_input: {}".format(a_input.size()))
        e = self.leakyrelu(torch.matmul(a_input, self.a).squeeze(2).float())
        log.info("e: {}".format(e.size()))
...
class DeepReservoir(torch.nn.Module):
    def __init__(self, input_size=1, tot_units=100, n_layers=1, concat=False,
        ...
        hyperparameter values (i.e., same number of recurrent neurons, spectral
        radius, etc. ).
        :param input_size:
...
class DeepReservoirPredictor(torch.nn.Module):
    def __init__(self, input_size, feature_dims, units=100, layers=5,
        ...
        """ SimpleDeepESNDynamicsPredictor. The model can be trained to predict
        arbitrarily long input time-series. The architecture contains a
        SimpleDeepReservoirLayer followed by a Dense layer for regression.
        :param input_size: the size of input, should match the size of the corresponding tensor.
        :param feature_dims: number of dimensions in the feature
        :param units:
        :param layers:
        :param concat:
        ...
        input_size = self.hidden.layers_units * 5
        ...
        self.output = torch.nn.Linear(input_size, self.feature_dims)
...
class ReservoirCell(torch.nn.Module):
    def __init__(self, input_size, units, input_scaling=1., spectral_radius=0.99,
        ...
        Originally from https://github.com/Pervasive-AI-Lab/ContinualLearning-EchoStateNetworks/ with modifications
    ...
    def forward(self, xt, h_prev):
        ...
        input_part = torch.matmul(xt, self.kernel)
        # print("input_part: ")
        # print(input_part.size())
        # state_part = torch.mm(h_prev, self.recurrent_kernel)
        state_part = torch.matmul(h_prev, self.recurrent_kernel)
        # print("state_part: ")
        # print(state_part.size())
        # print("self.bias: ")
...
class ReservoirLayer(torch.nn.Module):
    ...
    def init_hidden(self, batch_size, num_nodes):
        return torch.zeros(batch_size, num_nodes, self.net.units)
    ...
    def forward(self, x, h_prev=None):
        ...
        if h_prev is None:
            h_prev = self.init_hidden(x.size()[0], x.size()[2]).to(x.device)
