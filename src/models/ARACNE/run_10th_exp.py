...
srun_format = "srun -N 1 -n 1 --exclusive --unbuffered Rscript test.R --save-folder=\"./results/20230422_004\" --b-time-steps=49 --b-network-type=\"%s\" --b-simulation-type=\"netsims\" --b-suffix=\"%s\""
...
with open("run_10th_exp.sh", "w") as f:
    ...
    for edge_fn in list_experiments:
        ...
        for dirpath, subdirs, files in os.walk("./results/"):
            for x in files:
                if (x.endswith(suffix+".csv") or x.endswith(suffix.replace("r1","r2")+".csv") or x.endswith(suffix.replace("r1","r3")+".csv")) and network_type in x and "004" not in dirpath:
