...
def load_nri_benchmark(data: dict, size: int, args):
    """
    Load Springs / Charged dataset.
    ...
    """
    # edge list of a fully-connected graph
    a = list(permutations(range(size), 2))
    if args.data_path != '':
        for i in range(size):
            a.append((i, i))
        a = sorted(a)
...
def load_netsims_benchmark(data: dict, size: int, args):
    """
    Load Springs / Charged dataset.
    ...
    # for spring and charged
    # return maximum and minimum values of each input dimension in order to map normalized data back to the original space
    data, max_min = bold_(data)
    return data, es, max_min


def load_netsims(data: dict, size: int):
    """
    Load Springs / Charged dataset.
...
def bold_(data: dict):
    ...
    for key, value in data.items():
        # normalize the bold
        data[key][1][:, :, :, :1] = normalize(value[1][:, :, :, :1], loc_max, loc_min)

    return data, (loc_max, loc_min, loc_max, loc_min)
