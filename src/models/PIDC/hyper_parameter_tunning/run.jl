...
@add_arg_table s begin
	...
	arg_type = String
	default = "/shared/projects/BSIMDS/src/data/Local_Project/src/simulations/"
    "--save-folder"
...
ARGS= ["--save-folder", ".", "--b-network-type", "chemical_reaction_networks_in_atmosphere", "--b-directed", "--b-simulation-type", "netsims", "--b-suffix", "test_netsims15r1.npy"]
...
# model setting
for normalizer in ["none", "symlog", "unitary", "z-score"] 
    for discretizer in ["uniform_width", "uniform_count"]
        for estimator in ["maximum_likelihood", "miller_madow", "dirichlet", "shrinkage"]
            for number_of_bins in [4, 5, 10, 20, 100, 200, 500, 1000]
                ...
                for edges_fn in list_experiment
                    ...
                    if (normalizer == "symlog")
                        data = sym_log_normalizer(data)
                    elseif (normalizer == "unitary")
                        data = unitary_normalizer(data)
                    elseif (normalizer == "z-score")
                    ...
                    out_file_name = "inferred_network_$(normalizer)_$(discretizer)_$(estimator)_$(number_of_bins).csv"
                    touch(out_file_name)
