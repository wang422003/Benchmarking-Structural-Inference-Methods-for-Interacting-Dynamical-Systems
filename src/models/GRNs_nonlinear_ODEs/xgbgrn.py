...
class xgbgrn:
    ...
    def get_links(self, VIM, gene_names, regulators, sort=True, file_name=None):
    ...
    def estimate_degradation_rates(self, TS_data,time_points):
    ...
    def  get_importances(self, TS_data, time_points, alpha="from_data",  SS_data=None, gene_names=None,regulators='all', param={}):
        ...
        if alpha == "from_data":
            alphas = self.estimate_degradation_rates(TS_data,time_points)
        else:
            alphas = [alpha] * ngenes
        ...
        for i in range(ngenes):
            print("Running gene", i)
            input_idx = idx.copy()
            if i in input_idx:
                input_idx.remove(i)
            vi = self.get_importances_single(TS_data, time_points, alphas[i], input_idx, i, SS_data, param)
            VIM[i,:] = vi
    ...
    def get_importances_single(self, TS_data, time_points, alpha, input_idx, output_idx, SS_data, param):
    ...
    def get_scores(self, VIM, gold_edges, gene_names, regulators):
