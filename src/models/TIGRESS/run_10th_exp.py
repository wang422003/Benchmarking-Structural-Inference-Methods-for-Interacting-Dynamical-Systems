...
srun_format = "srun -N 1 -n 1 --cpus-per-task $SLURM_CPUS_PER_TASK --exclusive --unbuffered Rscript run.R --save-folder=\"./results/20230422_004\" --b-time-steps=49 --b-network-type=\"%s\" --b-simulation-type=\"netsims\" --b-suffix=\"%s\""
sbatch_header = """#!/bin/bash -l
#SBATCH -J bsimds_TIGRESS_004
#SBATCH -t 2-00:00:00
#SBATCH -N 50
...
with open("run_10th_exp.sh", "w") as f:
    ...
    for edge_fn in list_experiments:
        ...
        list_file = []
        for x in glob.glob(re.sub(r"r\d", "r?", f"./results/20230422_00*/*{network_type}*{suffix}.csv")):
            list_file.append(os.path.abspath(x))
        assert len(list_file) == 9, glob.glob(re.sub(r"r\d", "r?", f"./results/20230422_00*/*{network_type}*{suffix}.csv"))
